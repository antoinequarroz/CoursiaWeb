...
const mediaStats = computed(() => ({
  orphans: orphanAssets.value.length,
  renditions: recipeMediaRenditions.length,
  minSize: `${recipeMediaDimensions.minWidth}×${recipeMediaDimensions.minHeight}`,
  maxSize: readableMaxSize.value,
}))
...
const validationIssues = computed(() => {
  ...
  if (!recipeMediaAllowedTypes.includes(form.mimeType)) issues.push('Type de fichier non autorisé.')
...
const workflowSteps = computed(() => [
  { label: 'Fichier', done: Boolean(selectedFile.value), detail: selectedFile.value?.name ?? 'Aucun fichier' },
  { label: 'Recette', done: Boolean(form.recipeId), detail: selectedRecipe.value?.title ?? 'Non associée' },
  { label: 'Droits', done: rightsComplete.value, detail: form.rights.license || 'Auteur, source, licence' },
  { label: 'Accessibilité', done: Boolean(form.altText?.trim()), detail: form.altText || 'Texte alternatif requis avant publication' },
])
...
<template>
  <section class="admin-page">
    <div class="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
      <div>
        <p class="text-xs font-semibold uppercase tracking-[0.24em] text-coursia-primary">COUR-99 · Storage</p>
        ...
        <p class="mt-2 max-w-3xl text-sm leading-6 text-coursia-muted">
          Import, validation des droits, texte alternatif et publication contrôlée des images utilisées par le web et l’application mobile.
        </p>
      </div>
    ...
    <div class="grid gap-3 md:grid-cols-4">
      ...
      <article class="admin-stat-card">
        ...
        <strong>{{ mediaStats.minSize }}</strong>
      </article>
      <article class="admin-stat-card">
        <span>Renditions</span>
        <strong class="text-coursia-primary">{{ mediaStats.renditions }}</strong>
      </article>
      <article class="admin-stat-card">
        <span>Orphelins</span>
        <strong class="text-coursia-warning">{{ mediaStats.orphans }}</strong>
      </article>
    </div>
      ...
      <section class="grid gap-4">
        <form class="rounded-2xl border border-coursia-border bg-coursia-surface p-4 shadow-coursia-sm" @submit.prevent="uploadMedia()">
          <div class="flex flex-col justify-between gap-3 md:flex-row md:items-start">
            <div>
              <p class="text-xs font-semibold uppercase tracking-[0.18em] text-coursia-primary">Validation privée</p>
              <h2 class="mt-2 text-lg font-semibold text-coursia-text">Image et métadonnées</h2>
              <p class="mt-2 text-sm text-coursia-muted">
                Le fichier reste privé tant que les droits, la recette cible et l’accessibilité ne sont pas validés.
              </p>
            </div>
            <BaseBadge tone="neutral">{{ selectedMediaLabel }}</BaseBadge>
          </div>

          <label class="mt-4 grid cursor-pointer gap-2 rounded-2xl border border-dashed border-coursia-border bg-coursia-surface-muted p-5 text-sm text-coursia-muted transition hover:border-coursia-primary hover:bg-coursia-surface">
            <span class="font-semibold text-coursia-text">Importer une photo</span>
            <span>JPEG, PNG ou WebP. Maximum {{ mediaStats.maxSize }}. Dimensions lues automatiquement.</span>
            <input type="file" accept="image/jpeg,image/png,image/webp" class="sr-only" @change="onFileSelected">
            <span v-if="selectedFile" class="mt-1 break-all text-xs font-semibold text-coursia-primary">{{ selectedFile.name }}</span>
          </label>
          ...
          <div class="mt-4 grid gap-4 md:grid-cols-2">
            <label class="grid gap-1.5 text-xs font-semibold text-coursia-text md:col-span-2">
              ...
              <span v-if="selectedRecipe" class="truncate text-xs font-medium text-coursia-muted">
                Statut recette : {{ selectedRecipe.status }} · {{ selectedRecipe.id }}
              </span>
            </label>
          ...
          <div class="mt-4 grid gap-4 lg:grid-cols-2">
            <section class="rounded-2xl border border-coursia-border bg-coursia-surface-muted p-4">
              <h3 class="text-sm font-semibold text-coursia-text">Recadrage</h3>
          ...
          <section class="mt-4 rounded-2xl border border-coursia-border p-4">
            ...
            <div class="mt-3 grid gap-3 md:grid-cols-3">
              ...
              <input v-model="form.altText" class="md:col-span-3" placeholder="Texte alternatif lisible avant publication">
            </div>
          </section>
      ...
      <aside class="grid gap-4">
        <article class="rounded-2xl border border-coursia-border bg-coursia-surface p-4 shadow-coursia-sm">
          <div class="flex items-start justify-between gap-3">
            <div>
              <h2 class="text-base font-semibold text-coursia-text">Aperçu média</h2>
              <p class="mt-1 text-xs text-coursia-muted">Simulation du visuel utilisé par les fiches recette.</p>
            </div>
            <BaseBadge :tone="statusTone(form.status)">{{ form.status }}</BaseBadge>
          </div>
          <div class="mt-4 overflow-hidden rounded-2xl bg-coursia-surface-muted">
            ...
            >
            <div v-else class="grid aspect-[4/3] place-items-center bg-[radial-gradient(circle_at_30%_20%,rgba(34,197,94,0.18),transparent_30%),linear-gradient(135deg,#fff7ed,#eef7f2)] text-sm text-coursia-muted">
              Aucun fichier sélectionné
            </div>
          </div>
          <dl class="mt-4 grid grid-cols-2 gap-2 text-xs">
            <div class="rounded-xl bg-coursia-surface-muted p-3">
              <dt class="font-semibold text-coursia-muted">Poids</dt>
              <dd class="mt-1 font-semibold text-coursia-text">{{ formattedFileSize }}</dd>
            </div>
            <div class="rounded-xl bg-coursia-surface-muted p-3">
              <dt class="font-semibold text-coursia-muted">Dimensions</dt>
              <dd class="mt-1 font-semibold text-coursia-text">{{ form.width }}×{{ form.height }}</dd>
            </div>
          </dl>
        </article>

        <article class="rounded-2xl border border-coursia-border bg-coursia-surface p-4 shadow-coursia-sm">
          <h2 class="text-base font-semibold text-coursia-text">Renditions prévues</h2>
      ...
      <div class="flex flex-col justify-between gap-3 border-b border-coursia-border px-4 py-3 md:flex-row md:items-center">
        <div>
          ...
          <p class="mt-1 text-xs text-coursia-muted">
            Médias sans recette ou marqués orphelins avant nettoyage. Ces fichiers ne doivent pas alimenter l’application.
          </p>
        </div>
        <BaseBadge tone="warning">{{ loading ? 'Chargement' : `${orphanAssets.length} fichier(s)` }}</BaseBadge>
      </div>
        ...
        <table class="min-w-[54rem]">
          ...
          <tbody>
            <tr v-for="asset in orphanAssets" :key="String(asset.id ?? asset.private_path ?? asset.path)">
              <td class="font-semibold text-coursia-text">{{ asset.private_path || asset.path || asset.id }}</td>

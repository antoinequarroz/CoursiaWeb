...
const loadPreview = async () => {
  ...
  try {
    ...
    feedback.value = 'AperÃ§u mobile et web chargÃ©.'
  } catch {
    feedback.value = 'Impossible de charger lâ€™aperÃ§u de publication.'
  } finally {
    loading.value = false
  }
}
...
const runAction = async (action: 'submit-review' | 'publish' | 'unpublish') => {
  ...
  try {
    ...
    recipe.value = response.data ?? null
    preview.value = response.preview ?? null
    feedback.value = response.behavior ?? 'Workflow de publication mis Ã  jour et historisÃ©.'
  } catch {
    feedback.value = 'Action refusÃ©e : vÃ©rifie les champs bloquants et les droits de publication.'
  } finally {
    loading.value = false
  }
}
  ...
  <section>
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <h1 class="mt-2 text-3xl font-black">PrÃ©visualisation et publication des recettes</h1>
        <p class="mt-3 max-w-3xl text-coursia-muted">
          ContrÃ´le des Ã©tats brouillon, en validation, publiÃ© et archivÃ© avec aperÃ§u mobile/web,
      ...
      <div class="rounded-full bg-coursia-surface-muted px-4 py-2 text-sm font-black">
        DÃ©publier : retour en brouillon
      </div>
    </div>
    ...
    <form class="mt-8 grid gap-4 rounded-[1.4rem] border border-coursia-border bg-coursia-surface p-5 lg:grid-cols-[1fr_1fr_auto]" @submit.prevent="loadPreview">
      ...
      <label class="grid gap-2 text-sm font-bold">
        ...
        <input v-model="reason" class="rounded-coursia-md border border-coursia-border bg-coursia-background px-4 py-3" placeholder="Correction, validation finale, dÃ©publication..." />
      </label>
      <BaseButton class="self-end" type="submit" :disabled="loading">Charger lâ€™aperÃ§u</BaseButton>
    </form>
      ...
      <div class="rounded-[1.4rem] border border-coursia-border bg-coursia-surface p-5">
        ...
        <div class="mt-6 rounded-2xl bg-coursia-background p-4">
          ...
          <p class="mt-2 text-sm text-coursia-muted">
            Chaque passage en validation, publication ou dÃ©publication est enregistrÃ© avec auteur,
            ancien Ã©tat, nouvel Ã©tat, raison et snapshot de recette.
          </p>
        </div>
      </div>
    </section>
    ...
    <section class="mt-8 grid gap-5 lg:grid-cols-2">
      <article class="rounded-[2rem] border border-coursia-border bg-coursia-surface p-5 shadow-soft">
        <p class="text-sm font-black uppercase tracking-[0.18em] text-coursia-muted">AperÃ§u fiche mobile</p>
        <div class="mx-auto mt-5 max-w-[320px] rounded-[2.2rem] border border-coursia-border bg-coursia-background p-4">
          <div class="rounded-[1.6rem] bg-coursia-surface-muted p-5">
            ...
            <p class="mt-2 text-sm text-coursia-muted">{{ preview?.mobile.subtitle || 'CatÃ©gories' }}</p>
      ...
      <article class="rounded-[2rem] border border-coursia-border bg-coursia-surface p-5 shadow-soft">
        <p class="text-sm font-black uppercase tracking-[0.18em] text-coursia-muted">AperÃ§u fiche web</p>
        <div class="mt-5 rounded-[1.6rem] bg-coursia-background p-6">
          ...
          <p class="mt-5 text-sm text-coursia-muted">Source : {{ preview?.web.source ?? 'Ã€ complÃ©ter' }}</p>
